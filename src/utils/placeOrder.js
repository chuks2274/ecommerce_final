...
export const placeOrder = async (uid, cartItems, dispatch) => {
    try {
        ...
        // Notify admin users
        try {
            const itemSummaries = cartItems.map((item) => `${item.title} $${item.price.toFixed(2)} × ${item.quantity}`);
            const adminMessage = `📦 New order ${orderId} placed by user ${uid}. Status: pending. Items: ${cartItems.length}, Quantity: ${totalQuantity}, Total: $${total.toFixed(2)}. Order details: ${itemSummaries.join(" | ")}`;
            const usersRef = collection(db, "users");
            const adminQuery = query(usersRef, where("role", "==", "admin"));
            const adminSnapshot = await getDocs(adminQuery);
            const adminNotifications = adminSnapshot.docs.map((adminDoc) => addDoc(collection(db, "notifications"), {
                userId: adminDoc.id,
